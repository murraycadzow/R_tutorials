...
TWO GROUPS   
```{r}
mydata = read.delim(file = "dummy_data2.txt", header = TRUE, sep = "\t", stringsAsFactors=FALSE)
...
t.test(vinha$UricAcid_B,vinha$UricAcid_30,paired=TRUE)
```
...
```{r}
t.test(vinha$UricAcid_B,vinha$UricAcid_30,paired=FALSE)
```
...
```
   ...
   - Estimate = a beta value and centres around 0
      - [< 0] - as one variable goes up the other goes down
      - [ 0 ] - no relationship at all
      - [> 0] - as one variable goes up the other one does too
