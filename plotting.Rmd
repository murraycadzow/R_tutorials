...
## Generate 2-way tabe for ethnicity and gout
table(ETHCLASS, GOUT)

## Represent this information as a (stacked) barplot
barplot( table(ETHCLASS, GOUT), xlab = "Gout status", ylab = "Frequency" )

## Same information, unstacked
barplot( table(ETHCLASS, GOUT), xlab = "Gout status", ylab = "Frequency", beside = TRUE )
...
## Then make a barplot
barplot( t(table(ETHCLASS, GOUT)), beside = TRUE )

## What if we're more interested in proportions?
prop.table( table(ETHCLASS, GOUT) )

## Make the corresponding barplot
barplot( prop.table(table(ETHCLASS, GOUT)) )

## Or we can make the columns sum to one
prop.table( table(ETHCLASS, GOUT), 2)
barplot( prop.table(table(ETHCLASS, GOUT), 2) )
```
